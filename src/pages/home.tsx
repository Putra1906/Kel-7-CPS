...
const ardatain: Object = JSON.parse(data)
const waktu = Object.keys(ardatain)

function Tabling({ sepuluhan }) {
    const dataon: string = JSON.stringify(ardatain[waktu[sepuluhan]])
    const ardataon: Object = JSON.parse(dataon)
    const nilai = Object.values(ardataon)
    return (
        <>
            <tbody>
                <tr>
                    <th scope="row">{waktu[sepuluhan].slice(2)}</th>
                    <td>{nilai[0]}</td>
                    <td>{nilai[1]}</td>
                </tr>
            </tbody>
